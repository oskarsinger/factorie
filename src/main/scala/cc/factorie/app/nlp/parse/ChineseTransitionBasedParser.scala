...
//    featureGenerators.foreach(f => features += f.apply(state))
  ...
  def test(testSentences:Seq[Sentence]): (Double, Double, Double, Double) = {
    ...
    val pred = testSentences.map(_.attr[ChineseParseTree])
    (ChineseParserEval.calcLas(pred), ChineseParserEval.calcUas(pred), totalTokens*1000.0/totalTime, totalSentences*1000.0/totalTime)
  }
  ...
  def generateDecisions(ss: Iterable[Sentence], mode: Int, nThreads: Int): Iterable[ParseDecisionVariable] = {
    val decs = cc.factorie.util.Threading.parMap(ss, nThreads)(s => {
      val oracle: ChineseNonProjectiveOracle = {
        if (mode == ParserConstants.TRAINING) new ChineseNonprojectiveGoldOracle(s)
        else new ChineseNonprojectiveBoostingOracle(s, classify)
      }
  ...
  def postAttrs = Seq(classOf[ChineseParseTree])
  override def tokenAnnotationString(token:Token): String = {
    val sentence = token.sentence
    val pt = if (sentence ne null) sentence.attr[ChineseParseTree] else null
    if (pt eq null) "_\t_"
  ...
  def process(s: Sentence): Sentence = {
    println(s.attr.contains[ChineseParseTree])
...
//          println()
  ...
  trait ChineseNonProjectiveOracle {
    import ParserConstants._
    ...
    def getSimpleDepArcs = sentence.chineseParse.targetParents.map(_ + 1).zip(sentence.chineseParse.labels.map(_.target.value.category))
  ...
  class ChineseNonprojectiveGoldOracle(val sentence: Sentence) extends ChineseNonProjectiveOracle {
  ...
  class ChineseNonprojectiveBoostingOracle(val sentence: Sentence, basePredict: ParseDecisionVariable => ParseDecision) extends ChineseNonProjectiveOracle {
    def predict(decisionVariable: ParseDecisionVariable): ParseDecision = {
  ...
  class ParseState(var stack: Int, var input: Int, val reducedIds: collection.mutable.HashSet[Int], sentence: Sentence) {
    private def depToken(token: Token, idx: Int, state: ParseState) = new DepToken(form = token.string, lemma = token.lemmaString, pos = token.chinesePosTag.categoryValue, thisIdx = idx, state = state)
...
object ChineseTransitionBasedParserTrainer extends cc.factorie.util.HyperparameterMain {
  def evaluateParameters(args: Array[String]) = {
    val opts = new ChineseTransitionBasedParserArgs
    ...
    def testSingle(c: ChineseTransitionBasedParser, ss: Seq[Sentence], extraText: String = ""): Unit = {
      if (ss.nonEmpty) {
        println(extraText + " " + c.testString(ss))
      }
    }

    def testAll(c: ChineseTransitionBasedParser, extraText: String = ""): Unit = {
      println("\n")
    ...
    val c = new ChineseTransitionBasedParser
...
//    })
    ...
    if (opts.saveModel.value) {
      ...
      val d = new ChineseTransitionBasedParser
      d.deserialize(new java.io.File(modelUrl))
      testSingle(d, testSentences, "Post serialization accuracy ")
    }
    val testLAS = ChineseParserEval.calcLas(testSentences.map(_.attr[ChineseParseTree]))
...
object ChineseTransitionBasedParserTester {
...
object ChineseTransitionBasedParserOptimizer {
  def main(args: Array[String]) {
    val opts = new ChineseTransitionBasedParserArgs
    opts.parse(args)
