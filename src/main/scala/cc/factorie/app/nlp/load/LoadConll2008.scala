...
import cc.factorie.app.nlp.pos.{PennPosDomain, PosTag}
...
object LoadConll2008 {
  ...
  def fromFilename(filename:String): Seq[Document] = {
    ...
    document.annotators(classOf[pos.PosTag]) = UnknownDocumentAnnotator.getClass // register that we have POS tags
    ...
    for (line <- source.getLines()) {
      if (line.length < 2) { // Sentence boundary
        ...
        token.attr += PennPosDomain.newPosTag(token, partOfSpeech)
        if (loadLemma)
