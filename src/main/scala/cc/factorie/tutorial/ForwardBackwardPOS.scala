...
import cc.factorie.app.nlp.pos.{PosTag, LabeledPosTag, PennPosDomain}
...
object ForwardBackwardPOS {
  ...
  object PosModel extends ChainModel[LabeledPosTag,PosFeatures,Token](PennPosDomain, PosFeaturesDomain, l => l.token.attr[PosFeatures], l => l.token, t => t.attr[LabeledPosTag])
  ...
  def predictSentence(s: Sentence): Unit = predictSentence(s.tokens.map(_.attr[LabeledPosTag]))
  def predictSentence(vs: Seq[LabeledPosTag], oldBp: Boolean = false): Unit =
  ...
  def train(
    ...
    val examples = sentenceTags.map(s => new PosModel.ChainLikelihoodExample(s.asInstanceOf[Seq[LabeledPosTag]]))
  ...
  def test(documents: Seq[Document], label: String = "test"): Unit = {
    ...
    val labels = sentences.map(_.tokens).flatMap(_.map(t => t.attr[LabeledPosTag]))
  ...
  def process(document: Document): Unit = {
    ...
    if (document.tokens.head.attr.get[PosTag] == None) {
  ...
  def labelMaker(t: Token, l: String = defaultCategory) = PennPosDomain.newLabeledPosTag(t, l)
