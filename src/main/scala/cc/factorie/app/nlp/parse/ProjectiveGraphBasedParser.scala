...
class ProjectiveGraphBasedParser extends DocumentAnnotator {
  ...
  def getTokenFeatureVector(t: Token): TensorVar = {
    ...
    val tWord = t.string
    val tPos = t.attr[PosTag].categoryValue
    f += "TOKENPOS="+tPos
    f += "TOKENWORD="+tWord
  ...
  def getParentFeatureVector(p: Token): TensorVar = {
    ...
    val pPos = if (p ne null) p.attr[PosTag].categoryValue else "ROOTPOS"
    f += "PARENTPOS="+pPos
    f += "PARENTWORD="+pWord
  ...
  def getPairwiseFeatureVector(t: Token, p: Token): TensorVar = {
    ...
    if ((p ne null) && (t ne null)) {
      ...
      while (x.sentenceNext ne second) {
        x = x.sentenceNext
        f += dir+"BETWEENPOS="+pPos+"&"+x.attr[PosTag].categoryValue+"&"+tPos
      }
      val prevHeadPos = if (p.sentenceHasPrev) p.sentencePrev.attr[PosTag].categoryValue else "NOPREV"
      val prevTokPos = if (t.sentenceHasPrev) t.sentencePrev.attr[PosTag].categoryValue else "NOPREV"
      val nextHeadPos = if (p.sentenceHasNext) p.sentenceNext.attr[PosTag].categoryValue else "NONEXT"
      val nextTokPos = if (t.sentenceHasNext) t.sentenceNext.attr[PosTag].categoryValue else "NONEXT"
  ...
  def prereqAttrs: Iterable[Class[_]] = List(classOf[Sentence], classOf[pos.PosTag]) // TODO Also TokenLemma?  But we don't have a lemmatizer that matches the training data
