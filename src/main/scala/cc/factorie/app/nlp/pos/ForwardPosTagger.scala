...
/** Infrastructure for building and remembering a list of training data words that nearly always have the same POS tag.
    ...
    def computeAmbiguityClasses(tokens: Iterable[Token]) = {
      ...
      tokens.foreach(t => {
        ...
        if (t.attr[PosTag] eq null) {
          ...
          throw new Error("Found training token with no PosTag.")
        }
        ...
        wordCounts(lemma) += 1
        posCounts(lemma)(t.attr[PosTag].intValue) += 1
      })
      lemmas.foreach(w => {
  ...
  def features(token:Token, lemmaIndex:Int, lemmas:Lemmas): SparseBinaryTensor1 = {
    ...
    def posTagAtOffset(offset:Int): String = { val t = token.next(offset); "P@"+offset+(if (t ne null) t.attr[PosTag].categoryValue else null) }
...
//    addFeature("SUFFIX4@2="+takeRight(Wp2, 4))
  ...
  class SentenceClassifierExample(val tokens:Seq[Token], model:LinearMulticlassClassifier, lossAndGradient: optimize.OptimizableObjectives.Multiclass) extends optimize.Example {
    def accumulateValueAndGradient(value: DoubleAccumulator, gradient: WeightsMapAccumulator) {
      ...
      for (index <- 0 until tokens.length) {
        ...
        val posLabel = token.attr[LabeledPosTag]
  ...
  def predict(tokens: Seq[Token]): Unit = {
    ...
    for (index <- 0 until tokens.length) {
      val token = tokens(index)
      if (token.attr[PosTag] eq null) token.attr += PennPosDomain.newPosTag(token, "NNP")
      val l = lemmatize(token.string).toLowerCase
      if (WordData.sureTokens.contains(l)) {
        token.attr[PosTag].set(WordData.sureTokens(l))(null)
      } else {
        val featureVector = features(token, index, lemmaStrings)
        token.attr[PosTag].set(model.classification(featureVector).bestLabelIndex)(null)
      }
    }
  }
  ...
  def accuracy(sentences:Iterable[Sentence]): (Double, Double, Double, Double) = {
    ...
    sentences.foreach(s => {
      ...
      for (token <- s.tokens) {      
        tokenTotal += 1
        if (token.attr[LabeledPosTag].valueIsTarget) tokenCorrect += 1.0
  ...
  def train(trainSentences:Seq[Sentence], testSentences:Seq[Sentence], lrate:Double = 0.1, decay:Double = 0.01, cutoff:Int = 2, doBootstrap:Boolean = true, useHingeLoss:Boolean = false, numIterations: Int = 5, l1Factor:Double = 0.000001, l2Factor:Double = 0.000001)(implicit random: scala.util.Random) {
    ...
    if (false) {
      ...
      for (s <- testSentences) {
        for (t <- s.tokens) { val p = t.attr[LabeledPosTag]; source.println("%s %20s  %6s %6s".format(if (p.valueIsTarget) " " else "*", t.string, p.target.categoryValue, p.categoryValue)) }
        source.println()
      }
      source.close()
    }
  ...
  def postAttrs: Iterable[Class[_]] = List(classOf[PosTag])
  override def tokenAnnotationString(token:Token): String = { val label = token.attr[PosTag]; if (label ne null) label.categoryValue else "(null)" }
}
