...
object PosBasedNounPhraseFinder extends DocumentAnnotator {
  def process(document:Document): Document = {
    ...
    for (section <- document.sections; token <- section.tokens) {
      ...
      val posPrefix = token.attr[pos.PennPosDomain.Tag].categoryValue.take(2)
      if (posPrefix == "NN" || posPrefix == "PR" || (posPrefix == "JJ" && token.hasNext && token.next.attr[pos.PennPosDomain.Tag].categoryValue.take(2) == "NN")) {
        if (tempSpan eq null) tempSpan = new  NounPhrase(section, token.position, 1)
        else tempSpan.append(1)(null)
      } else if (tempSpan ne null) {
        if (token.string == "-" && token.hasNext && token.next.attr[pos.PennPosDomain.Tag].categoryValue.take(2) == "NN") tempSpan.append(1)(null) // Handle dashed nouns
  ...
  def prereqAttrs: Iterable[Class[_]] = List(classOf[pos.PennPosDomain.Tag])
