...
object LoadWSJMalt {
  ...
  def fromLines(lines:Iterator[String], filename:String = "?UNKNOWN?", loadLemma:Int = AnnotationTypes.GOLD, loadPos:Int = AnnotationTypes.GOLD, loadParse:Int = AnnotationTypes.GOLD, loadNer:Boolean = true, nerBilou:Boolean = false): Seq[Document] = {
    ...
    if (loadPos != AnnotationTypes.NONE) document.annotators(classOf[pos.PosTag]) = UnknownDocumentAnnotator.getClass // register that we have POS tags
    ...
    for (line <- lines) {
      if (line.length < 2) { // Sentence boundary
        ...
        loadPos match{
	      case AnnotationTypes.GOLD => {token.attr += PennPosDomain.newLabeledPosTag(token, if (goldPartOfSpeech == "XX") "PUNC" else goldPartOfSpeech)}
	      case AnnotationTypes.AUTO => {token.attr += PennPosDomain.newLabeledPosTag(token, if (autoPartOfSpeech == "XX") "PUNC" else autoPartOfSpeech)}
